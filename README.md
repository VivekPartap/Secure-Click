...
- 🤖 **Machine Learning Detection**: Uses XGBoost model trained on phishing datasets
...
2. This will automatically set the API key and start the server
...
1. **Automatic Scanning**: The extension automatically scans URLs as you browse. When you visit a website, it will analyze the URL in the background.
...
- **Model:**
  - XGBoost classifier trained on combined datasets in `data/`
  - Output: probability \(p_{ml} ∈ [0,1]\) that the URL is suspicious
...
- Prerequisites:
  - Obtain an API key from Google Safe Browsing. See Google’s documentation.
- Configure:
  - Set the environment variable before starting the backend:
    - Windows PowerShell: `$env:SAFE_BROWSING_API_KEY=\"YOUR_KEY\"`
    - macOS/Linux: `export SAFE_BROWSING_API_KEY=YOUR_KEY`
  - Optional: adjust cache TTL (default 3600 seconds): `SAFE_BROWSING_TTL_SECONDS=1800` (for 30 minutes)
- Behavior:
...
5. Save the model and vectorizer to `backend/models/`
...
- Uses XGBoost for machine learning classification
